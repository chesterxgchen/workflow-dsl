...
case class SparkSQL(master: String = "local[4]", appName: String = "defaults",
  ...
  def execute(query: String): String = {
    ...
    import sqlContext._
    val people = sc.textFile("/Users/chester/projects/spark/examples/src/main/resources/people.txt")
...
class ExternalDSL extends JavaTokenParsers {
  ...
  def runSrc = "run" ~> "workflow" ~> "with" ~> "workflowId" ~> workflowId
  def runSource = runSrc ~ atUri ~ authToken ~ opt(showProgress) ^^ {
    case workflowId ~ uri ~ authToken ~ None =>  s"$uri/$workflowId/run?token=$authToken"
    case workflowId ~ uri ~ authToken ~ Some(show) => s"$uri/$workflowId/run?token=$authToken$showProgress=$show"
  }
  ...
  //run flow
  def showProgress = stringLiteral
  def authToken = stringLiteral
  def workflowId = stringLiteral
